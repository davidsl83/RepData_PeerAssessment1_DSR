...
The `mean` is `r as.character(round(rmean,2))` the `median` is `r as.character(round(rmedian,2))` and the `standard deviation` is `r as.character(round(rsd,2))` .
...
The 5-minute interval, on average across all the days in the data set, containing the maximum number of steps is `r max_interval` which value is  `r max_interval_value`.
...
```
* The imputed data mean is `r as.character(round(rmean.i,2))` 
* The imputed data median is `r as.character(round(rmedian.i,2))` 
...
* The difference between total number of steps between imputed and non-imputed data is `r as.character(round(total_diff,2))`. Thus, there were `r as.character(round(total_diff,2))` more steps in the imputed data.
